...
namespace REvernus.ViewModels
{
    public class MarketOrdersViewerViewModel : BindableBase
    {
        ...
        private async Task LoadOrdersFromFiles()
        {
            ...
            {
                var orders = File.ReadAllLines(mostRecentOrderFile.FullName).Skip(1)
                    .Select(v => new CharacterMarketOrder().FromCsv(v)).ToList();
                var name = "";
                try
                {
        ...
        {
            var characterOrders = await SelectedCharacter.GetCharacterMarketOrdersAsync();
            var characterName = SelectedCharacter.CharacterName;
        ...
        {
            // Todo: this needs refactoring, really, really badly.
            IsEnabled = false;
            ...
            var toBeRemoved = CharacterToOrders[characterName]
                .Where(o => !characterOrders.Select(n => n.OrderId).Contains(o.OrderId)).ToList();
            var toBeAdded = characterOrders
                .Where(n => !CharacterToOrders[characterName].Select(o => o.OrderId).Contains(n.OrderId)).ToList();
            var toBeUpdated = CharacterToOrders[characterName]
                .Where(o => characterOrders.Select(n => n.OrderId).Contains(o.OrderId)).ToList();

            foreach (var characterMarketOrder in toBeRemoved)
                CharacterToOrders[characterName].Remove(characterMarketOrder);

            foreach (var characterMarketOrder in toBeAdded) CharacterToOrders[characterName].Add(characterMarketOrder);
            ...
            var authDto = new AuthDTO
            {
                ...
                Scopes = EsiScopes.Scopes.ToString()
            };

            foreach (var location in locations)
            {
                var orders = await Market.GetOrdersInStructure(location, authDto, typeIds.ToList());
        ...
        private async Task UpdateData(string characterName = "")
        {
                ...
                foreach (var charName in CharacterToOrders.Keys)
                    foreach (var characterOrder in CharacterToOrders[charName])
                    {
                        ...
                        var newOrder = new MarketOrderInfoModel(characterOrder,
                            App.AuthProvider.CharacterList.FirstOrDefault(c => c.CharacterName == characterName),
            ...
            else
            {
                ...
                foreach (var item in sellRemove) SellOrdersCollection.Remove(item);

                var buyRemove = BuyOrdersCollection.Where(o => o.Owner != characterName).ToList();
                foreach (var item in buyRemove) BuyOrdersCollection.Remove(item);

                CharacterToOrders[characterName].RemoveAll(o => o == null);
        ...
        private void SubscribeHotKeys()
        {
            ...
            {
                {
                    Combination.FromString(App.Settings.MarketOrdersTabSettings.MarketUpHotkey),
                    async () => await KeyBindMoveUp()
                },
                {
                    Combination.FromString(App.Settings.MarketOrdersTabSettings.MarketDownHotkey),
                    async () => await KeyBindMoveDown()
                }
            };
        ...
        private async Task MoveSelectedRow(Key direction)
        {
            try
            {
                ...
                if (nextIndex < 0 || nextIndex >= selectedGrid.Count) return;
                ...
                var character =
                    App.AuthProvider.CharacterList.FirstOrDefault(c => c.CharacterName == currentItem.Owner);
                ...
                Clipboard.SetText(Math
                    .Round(
                        currentItem.Order.IsBuyOrder == true
                            ? currentItem.BuyOrders[0].Price + App.Settings.MarketSettings.GetUndercut
                            : currentItem.SellOrders[0].Price - App.Settings.MarketSettings.GetUndercut, 2,
                        MidpointRounding.ToEven).ToString("F"));
