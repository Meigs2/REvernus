namespace REvernus.ViewModels
{
    ...
    public class MarketOrdersViewerViewModel : BindableBase
    {
        ...
        private async Task UpdateData(string characterName = "")
        {
            ...
            if (characterName == "")
            {
                foreach (var charName in CharacterToOrders.Keys)
                {
                    foreach (var characterOrder in CharacterToOrders[charName])
                    {
                        ...
                        if (!OrdersList.TryGetValue(characterOrder.LocationId, out var idsToOrders))
                            continue;
                        if (!idsToOrders.TryGetValue(characterOrder.TypeId, out var marketOrders))
                            continue;
            ...
            {
                if (marketOrderInfoModel.Escrow == null)
                    continue;
        ...
        private async Task UpdateOrders(List<CharacterMarketOrder> characterOrders, string characterName)
        {
            ...
            foreach (var oldOrder in toBeUpdated)
            {
                ...
                if (newOrder.Issued >= oldOrder.Issued)
                    continue;
        ...
        private async Task MoveSelectedRow(Key direction)
        {
            try
            {
                ...
                if (selectedGrid == null || currentItem == null)
                    return;
                ...
                var character =
                    App.CharacterManager.CharacterList.FirstOrDefault(c => c.CharacterName == currentItem.Owner);
                ...
                if (App.Settings.MarketOrdersTabSettings.PlayOrderChangedSound)
